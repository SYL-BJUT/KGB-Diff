"""
Baseline IP-Adapter inference for the Palmvein -> Palmprint direction (128 resolution).

Loads a fine-tuned IP-Adapter checkpoint produced by ``train_V2P_gen.py`` and
runs DDIM sampling to translate palmvein images into palmprint images. There
...
Stage-1 release: shipped alongside ``train_V2P_gen.py`` so reviewers can
reproduce the "no-retrieval" baseline outputs for the V2P direction.
"""
...
def _make_id(stem):
    ...
    Polyu palm_XXXXXX: palm_000307 -> palm_000307
    Polyu 平铺: PalmVein_003_2 -> PalmVein_003_2
...
class BaselineInferenceVein2Palm:
    """Baseline IP-Adapter inference (Palmvein -> Palmprint, no KG, 128 resolution)."""

    def __init__(self, sd_model_path, ip_adapter_path, image_encoder_path, resolution: int = 128, device="cuda"):
        self.device = device
    ...
    @torch.no_grad()
    def generate(self, palmvein_image, num_inference_steps=50, guidance_scale=1.0, seed=None):
        """
        生成掌纹

        Args:
            palmvein_image: PIL Image或路径
        ...
        """
        # 加载图像
        if isinstance(palmvein_image, str):
            palmvein_image = Image.open(palmvein_image).convert("RGB")

        # 编码掌静脉
        clip_image = self.clip_image_processor(images=palmvein_image, return_tensors="pt").pixel_values
        clip_image = clip_image.to(self.device)

        palmvein_embeds = self.image_encoder(clip_image).image_embeds
        palmvein_tokens = self.image_proj_model(palmvein_embeds)

        # 复制掌静脉tokens，让两对手都处理掌静脉（与训练时一致）
...
def main():
    parser = argparse.ArgumentParser(
        description="Baseline IP-Adapter inference for Palmvein -> Palmprint "
                    "at 128 resolution (no KG)."
    )
    ...
    parser.add_argument(
        ...
        help="Path to the trained IP-Adapter state_dict produced by train_V2P_gen.py "
    ...
    parser.add_argument(
        ...
        help="Directory of test palmvein images (input modality).",
    )
    parser.add_argument(
        "--output_dir", type=str, required=True,
        help="Directory to write generated palmprint images into.",
    )

    # ---- Sampling ----
    parser.add_argument(
        "--resolution", type=int, default=128,
        help="Output spatial resolution. Must be divisible by 8 (default: 128).",
    )
    parser.add_argument(
    ...
    inferencer = BaselineInferenceVein2Palm(
        ...
        for dp, _, fns in os.walk(args.input_dir):
            for fn in fns:
                if fn.lower().endswith(('.jpg', '.png', '.bmp')):
                    ...
                    stem = os.path.relpath(full, args.input_dir)
                    stem = stem.replace(os.sep, "_").rsplit(".", 1)[0]
                    test_images.append((full, stem))
    else:
        test_images = []
        for f in sorted(os.listdir(args.input_dir)):
            if f.lower().endswith(('.jpg', '.png', '.bmp')):
                test_images.append((os.path.join(args.input_dir, f), os.path.splitext(f)[0]))

    print(f"\nFound {len(test_images)} test images")
    print(f"Generating palmprint images (Palmvein -> Palmprint, baseline) ...\n")
    ...
    for i, (input_path, stem) in enumerate(test_images):
        ...
        generated_palmprint = inferencer.generate(
            palmvein_image=input_path,
            num_inference_steps=args.num_inference_steps,
            seed=args.seed + i,
        )

        generated_palmprint.save(output_path, format="PNG")
        print(f"  -> {output_path}")
